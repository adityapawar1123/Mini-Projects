...
if name.strip().lower() in ["i don't want to tell you my name", "i don't want to tell you", "i don't want to tell you my", "no", "nah", "nope", "i don't wanna tell you", "i don't wanna tell you my",  "i don't want to tell you my name", "i don't wanna tell you my name", "no thanks", "I ain't telling you my name"] :
 ...
 if 5 < time <= 12:
  tts(f"Good morning {name.capitalize()}!")

 elif 12 < time <= 17:
  tts(f"Good afternoon {name.capitalize()}!")
  
 elif 17 < time <= 22:
  tts(f"Good evening {name.capitalize()}!")
  
 else :
  tts(f"Looks like you're a night owl {name.capitalize()}!")

q2_category = [
   f"What genre of jokes do you wanna listen to {name.capitalize()}?👀",
   f"So you up for what kinda jokes today {name.capitalize()} ?",
   f"What are you staring at {name.capitalize()} ? Tell me the genre of jokes you wanna hear, bruh this is getting exhausting🙄",
   f"What's your mood today {name.capitalize()} ? What kinda jokes do you wanna hear?👀",
   f"Okay {name.capitalize()}, so what kind of jokes do you wanna hear?"
]
...
tts("Please speak the category of jokes you wanna hear")

category_chosen = speech_recog()

def get_joke_any(category=category_chosen.capitalize().strip()):
...
#check out learning.py for a better explanation with example
      ...
      engine.runAndWait()
      retry1 = speech_recog()
      print("We're facing speech recognition issues. If your answer is \"yes\", then please say:\n\t\"yes sir\\yes please\\yes joke teller\\yes jokingo\" instead \nThanks for your cooperation!")

      
      if retry1.lower().strip() not in ["ofcourse", "why not", "why not?", "obvious", "obviously", "absolutely", "yes motherfucker", "yes mf", "yes mother fucker", "yes", "yess", "yeah", "yos", "yoss","yes sir", "helia", "hell yeah", "yaa", "yup", "yupp", "ok", "okay", "okiess", "okie", "okies", "yes please", "yes joke tailor", "yes joke teller", "yes jokingo", "yes joking go"] :
        ...
        if retry2.lower().strip() not in ["ofcourse", "why not", "why not?", "obvious", "obviously", "absolutely", "yes motherfucker", "yes mf", "yes mother fucker", "yes", "yess", "yeah", "yos", "yoss","yes sir", "helia", "hell yeah", "yaa", "yup", "yupp", "ok", "okay", "okiess", "okie", "okies", "yes please", "yes joke tailor", "yes joke teller", "yes jokingo", "yes joking go"] :
...
if category_chosen.lower().strip() in ["dad jokes", "dad joke", "dads jokes", "dad's jokes", "dad's joke", "dads joke"] :
  joke_final = get_joke_any(category = "Pun") 
  engine.say(joke_final)
  print(f"Here's the joke:\n{joke_final}")
  engine.runAndWait()

else : 
  joke_final = get_joke_any(category = category_chosen.capitalize().strip()) 
  engine.say(joke_final)
  print(f"Here's the joke:\n{joke_final}")
  engine.runAndWait()
...
while True : 
  ...
  print("Do you wanna hear another joke?(yes/no): ")
  print("We're facing speech recognition issues. If your answer is \"yes\", then please say:\n\t\"yes sir\\yes please\\yes joke teller\\yes jokingo\" instead \nThanks for your cooperation!")

  joke_loop_input = speech_recog()
  ...
  if joke_loop_input.strip().lower() in [
    ...
    "ugh"] :
    not_funny_roasts = [  
    f"Not funny? Damn {name.capitalize()}, you must be fun at funerals.",  
    f"{name.capitalize()}, your personality is so dry, even the desert called to say 'chill'.",  
    f"If humor was oxygen, you'd be gasping for air right now {name.capitalize()}.",  
    f"Bro, even a rock has better comedic timing than you {name.capitalize()}.",  
    f"Not funny? Nah, {name.capitalize()}, your humor just got patch notes and still sucks.",  
    f"{name.capitalize()}, I'd say 'get a sense of humor' but that requires a personality upgrade first.",  
    f"Lmao {name.capitalize()}, I bet you read joke books like they're horror stories.",  
    f"Damn {name.capitalize()}, your existence is already tragic, and now your humor is too?",  
    f"Not funny? {name.capitalize()}, neither is your entire life but you don't see me complaining.",  
    f"{name.capitalize()}, you laugh like a Windows shutdown sound.",  
    f"Your humor is so dead, I'm sending my condolences {name.capitalize()}.",  
    f"Bruh, even cavemen had better jokes than you {name.capitalize()}.",  
    f"{name.capitalize()}, you probably think watching paint dry is peak entertainment.",  
    f"Bro, even NPCs react more than you do {name.capitalize()}.",  
    f"Your humor is like a Nokia 3310—old, unbreakable, and completely useless in today's world.",  
    f"{name.capitalize()}, your reaction time is slower than Internet Explorer loading a meme.",  
    f"Not funny? Damn, {name.capitalize()}, even the crickets are side-eyeing you.",  
    f"I'd suggest a sense of humor, {name.capitalize()}, but I don't think it's in stock for you.",  
    f"Lmaoo {name.capitalize()}, if laughter is medicine, you must be allergic.",  
    f"Bro, even Siri got better comedic timing than you {name.capitalize()}.",  
    f"{name.capitalize()}, I could explain the joke, but explaining jokes to NPCs isn't worth my time.",  
    f"Not funny? Alright, {name.capitalize()}, go ahead and tell a joke—oh wait, your life already is one.",  
    f"{name.capitalize()}, your humor is so broken, even an update wouldn't fix it.",  
    f"Lmaooo {name.capitalize()}, you got the personality of an Excel spreadsheet.",  
    f"Bro, I bet even AI-generated jokes are funnier than you {name.capitalize()}.",  
    f"{name.capitalize()}, even Google Translate has more expression than you.",  
    f"Not funny? Bro, your laugh button is more dysfunctional than your WiFi.",  
    f"{name.capitalize()}, your humor is so lifeless, even zombies wouldn't bite it.",  
    f"Lmaoo {name.capitalize()}, your comedic taste is as bland as boiled chicken.",
    f"Ohh that wasn't funny huh? You know what's funny {name.capitalize()}? Your life." ]  
  ...
  elif joke_loop_input.strip().lower() not in ["yes joke tailor", "ofcourse", "why not", "why not?", "obvious", "obviously", "absolutely", "yes motherfucker", "yes mf", "yes mother fucker", "yes", "yess", "yeah", "yeahh", "yos", "yoss", "yes sir", "yessir", "hell yeah", "helia", "yaa", "yup", "yupp", "ok", "okay", "okiess", "okie", "okies", "yes please", "yes joke teller", "yes jokingo", "yes joking go"] :
    exit_roasts = [
                   f"Okay whatever {name.capitalize()} you lame ass🙄", 
                   f"Fine, whatever {name.capitalize()}, go be boring somewhere else.",
                   f"Alright, party pooper {name.capitalize()}, go touch grass or something.",
                   f"Damn {name.capitalize()}, who hurt you?",
                   f"Okay fine, I didn't wanna tell you jokes anyway.🙄",
                   f"Bruh {name.capitalize()}, you got the personality of a dry biscuit.",
                   f"Alright, Mr./Ms. Fun Police, I'll stop.",
                   f"Your loss, {name.capitalize()}. Hope your life stays as dry as your sense of humor.",
                   f"Okay, I see how it is. I try to make you laugh, and you just reject me like my crush did.",
                   f"Damn, I thought we had something special, {name.capitalize()}. Guess not. 💔",
                   f"Alright {name.capitalize()}, go back to being a humorless NPC.",
                   ...
                   f"Aight {name.capitalize()}, go be a disappointment somewhere else.",
                   f"Wow {name.capitalize()}, rejecting jokes? That's crazy. Couldn't be me.",
   ...
   print("Do you want to continue with the same category of jokes?(yes/no): ")
   print("We're facing speech recognition issues. If your answer is \"yes\", then please say:\n\t\"yes sir\\yes please\\yes joke teller\\yes jokingo\" instead \nThanks for your cooperation!")

   
   q4_switch_category = speech_recog()
   if q4_switch_category.lower().strip() not in ["yes joke tailor", "ofcourse", "why not", "why not?", "obvious", "obviously", "absolutely", "yes motherfucker", "yes mf", "yes mother fucker", "yes", "yess", "yeah", "yeahh", "yos", "yoss", "yes sir", "yessir", "hell yeah", "helia", "yaa", "yup", "yupp", "ok", "okay", "okiess", "okie", "okies", "yes please", "yes joke teller", "yes jokingo", "yes joking go"] :
     tts("Select a category") 
     print("Available categories are:\n\t1. Programming\n\t2. Miscellaneous\n\t3. Dark\n\t4. Pun(dad jokes)\n\t5. Spooky\n\t6. Christmas\n\t7. Any(includes all categories)")
     print("Speak the category of jokes you wanna hear")
     
     switch_category_input = speech_recog().strip().capitalize()
     
     if switch_category_input.lower().strip() in ["dad jokes", "dad joke", "dads jokes", "dad's jokes", "dad's joke", "dads joke"] :
            joke_final = get_joke_any(category = "Pun")
            engine.say(joke_final)
            print(f"Here's the joke:\n{joke_final}")
            engine.runAndWait()
